/**
 * Created by tsesamli on 2018/11/24.
 */
...
class Tucao {
  ...
  getUrl(options = {}) {
    ...
    const params = Object.entries({
      scene, product_id, post_id, nickname, avatar, openid, debug
    })
    .filter(([_, value]) => !!value) //过滤 Falsy
    .map(([key, value]) => `${key}=${value}`).join('&') + `&extraData=${JSON.stringify(extraData) || ''}`;
